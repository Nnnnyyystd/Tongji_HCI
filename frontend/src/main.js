...
const THEME_KEY = 'foodmate_theme'
...
function applyTheme(theme) {
  const nextTheme = theme === 'forest' ? 'forest' : 'warm'
  ...
  return `
    <section class="card">
      <div class="section-heading">
        <h2>主题色</h2>
        <span>当前：${currentTheme() === 'forest' ? '森林绿' : '暖食色'}</span>
      </div>
      <div class="theme-options" role="radiogroup" aria-label="主题色">
        <label class="theme-option warm">
          <input type="radio" name="theme_choice" value="warm" ${currentTheme() === 'warm' ? 'checked' : ''} />
          <span class="theme-swatch"></span>
          <strong>暖食色</strong>
          <em>默认，偏暖更有食欲</em>
        </label>
        <label class="theme-option forest">
          <input type="radio" name="theme_choice" value="forest" ${currentTheme() === 'forest' ? 'checked' : ''} />
          <span class="theme-swatch"></span>
          <strong>森林绿</strong>
          <em>保留原有清爽配色</em>
        </label>
  ...
  <main class="phone-shell" aria-label="FoodMate demo">
    <section class="topbar">
      <div class="brand-title">
        <img class="app-icon" src="/foodmate-icon.svg" alt="FoodMate" />
        <div>
...
document.addEventListener('change', (event) => {
  ...
  if (event.target.matches('input[name="theme_choice"]')) {
    applyTheme(event.target.value)
    const currentLabel = event.target.closest('.card')?.querySelector('.section-heading span')
    if (currentLabel) currentLabel.textContent = `当前：${currentTheme() === 'forest' ? '森林绿' : '暖食色'}`
    if (activeChart) {
      const chartId = getCurrentRoute() === 'trend' ? 'trend-week-chart' : getCurrentRoute() === 'home' ? 'home-week-chart' : null
      if (chartId) loadWeekChart(chartId)
    }
  }
})
...
async function loadWeekChart(canvasId) {
  ...
  gradientFill.addColorStop(0, cssVar('--chart-fill-a'))
  gradientFill.addColorStop(0.62, cssVar('--chart-fill-b'))
  gradientFill.addColorStop(1, cssVar('--chart-fill-c'))

  const gradientStroke = canvasCtx.createLinearGradient(0, 0, ctx.width || 320, 0)
  gradientStroke.addColorStop(0, cssVar('--chart-stroke-a'))
  gradientStroke.addColorStop(1, cssVar('--chart-stroke-b'))
  ...
  activeChart = new Chart(ctx, {
      ...
      datasets: [
        {
          ...
          borderWidth: 2.5,
          pointRadius: 4,
          pointBackgroundColor: cssVar('--chart-point'),
          pointBorderColor: cssVar('--chart-stroke-a'),
          pointBorderWidth: 2,
          pointHoverRadius: 6,
          pointHoverBackgroundColor: cssVar('--chart-stroke-a'),
          pointHoverBorderColor: cssVar('--chart-point'),
          spanGaps: false,
        },
      ],
    },
    options: {
      ...
      scales: {
        x: {
          grid: { display: false },
          ticks: { color: cssVar('--chart-tick'), font: { size: 12 } },
          border: { display: false },
        },
        y: {
          min: 0,
          max: 100,
          ticks: { stepSize: 20, color: cssVar('--chart-tick'), font: { size: 12 } },
